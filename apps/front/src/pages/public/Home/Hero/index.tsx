...
export function Hero() {
  ...
  const [geoLocation, setGeoLocation] = useState<Address | null>(null);
  ...
  useEffect(() => {
    setGeoLocation(
      user?.address
        ? stringToAddress(user?.address)
        : localStorageAddress
          ? stringToAddress(localStorageAddress)
          : null,
    );
  }, [user, localStorageAddress]);

  const handleSaveLocation = useCallback(
    async (geoLocation: Address | null) => {
      if (geoLocation) {
        if (user) {
          mutate({
            address: addressToString(geoLocation),
          });
        } else {
          setLocalStorageAddress(addressToString(geoLocation));
        }
      }
    },
    [],
